...
const SERVICE_NAME_LOOKUP: Record<string, string> = Object.keys(SERVICE_DURATIONS).reduce(
  (acc, name) => {
...
function normalizeServiceToken(token: string) {
...
function splitServiceRequest(service: unknown) {
...
export function getPriceForService(service: unknown) {
  const parts = splitServiceRequest(service);
  if (parts.length === 0) return null;
