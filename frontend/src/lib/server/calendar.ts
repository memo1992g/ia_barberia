...
function normalizePrivateKey(key: unknown) {
...
function createCalendarClient() {
  ...
  if (!creds.client_email || !creds.private_key) {
    ...
    (error as Error & { statusCode?: number; userMessage?: string }).statusCode = 500;
    (error as Error & { statusCode?: number; userMessage?: string }).userMessage =
...
function buildBusyBlocks(items: any[] = []) {
  return items
    .map((item) => {
      const startValue =
...
async function fetchBusyBlocksForDate(calendar: any, dateString: string) {
  ...
  const calendarData = calendarId ? response.data.calendars?.[calendarId] : undefined;
  return calendarData?.busy || [];
}

function createMockAvailability(date: string, startTime: string, durationMinutes: number) {
  const occupied = startTime === "15:00";
  return {
    available: !occupied,
    message: occupied ? "Horario ocupado" : "Horario disponible",
    alternatives: occupied ? ["15:30", "16:00", "16:15"] : [],
  };
}

export function normalizeAppointmentPayload(payload: any, allowMissingConfirmation = false) {
  ...
  if (!service) {
    const error = new Error("Falta el servicio.") as Error & { statusCode?: number; userMessage?: string };
    error.statusCode = 400;
  ...
  if (!date || !startTime) {
    const error = new Error("Falta fecha u hora.") as Error & { statusCode?: number; userMessage?: string };
    error.statusCode = 400;
  ...
  if (!allowMissingConfirmation && (!customerName || !phone)) {
    const error = new Error("Faltan datos obligatorios.") as Error & {
      statusCode?: number;
      userMessage?: string;
    };
    error.statusCode = 400;
  ...
  if (customerName && customerName.length < 2) {
    const error = new Error("Nombre inválido.") as Error & { statusCode?: number; userMessage?: string };
    error.statusCode = 400;
  ...
  if (phone && !isValidPhoneInput(phone)) {
    const error = new Error("Teléfono inválido.") as Error & { statusCode?: number; userMessage?: string };
    error.statusCode = 400;
...
export async function checkAvailability(payload: any) {
  const normalized = normalizeAppointmentPayload(payload, true);
  const durationMinutes = normalized.durationMinutes ?? 0;

  if (useMocks()) {
    return createMockAvailability(normalized.date, normalized.startTime, durationMinutes);
  }

  const calendar = createCalendarClient();
  const calendarId = getCalendarId();
  if (!calendarId) {
    const error = new Error("Falta GOOGLE_CALENDAR_ID.") as Error & { statusCode?: number; userMessage?: string };
  ...
  try {
    ...
    const hasConflict = busyBlocks.some((busy: any) => {
...
export async function createAppointment(payload: any) {
  ...
  if (useMocks()) {
    ...
    if (!availability.available) {
      const error = new Error("Horario ocupado") as Error & { statusCode?: number; userMessage?: string };
    ...
    return {
      ...
      event: {
        summary: `Cita - ${normalized.service} - ${normalized.customerName}`,
  ...
  try {
    ...
    if (!rules.ok) {
      const error = new Error(rules.message) as Error & { statusCode?: number; userMessage?: string };
      error.statusCode = 400;
    ...
    const response = await calendar.events.insert({
      calendarId,
      requestBody: {
        summary: `Cita - ${service} - ${normalized.customerName}`,
        description: [
          `Cliente: ${normalized.customerName}`,
